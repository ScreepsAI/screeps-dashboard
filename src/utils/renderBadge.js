export default (badge, width, bgcolor = 'rgba(0,0,0,0)', outline = 'rgba(0,0,0,0)') => {
  width = width || 100;
  const badgePaths = getBadgePaths();
  const colors = getColors();
  const scale = 0.01 * width;
  ...
  let path2;
  let flip;
  if (typeof badge.color1 === 'number' && colors[badge.color1])
    badge.color1 = colors[badge.color1].rgb || '#FF0000';
  if (typeof badge.color2 === 'number' && colors[badge.color2])
    badge.color2 = colors[badge.color2].rgb || '#FF0000';
  if (typeof badge.color3 === 'number' && colors[badge.color3])
    badge.color3 = colors[badge.color3].rgb || '#FF0000';
  if (typeof badge.type === 'number') {
    const bp = badgePaths[badge.type];
    bp.calc(badge.param);
    path1 = bp.path1;
    path2 = bp.path2;
    flip = bp.flip;
